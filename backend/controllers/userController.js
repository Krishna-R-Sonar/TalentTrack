            ...
            if(resume){
                try {
                    ...
                    userData.resume = {
                        ...
                        // public_id: The unique identifier for the uploaded file in Cloudinary.
                        url: cloudinaryResponse.secure_url, // secure_url: The HTTPS URL to access the uploaded file.
                    };
                } catch (error) {
...
export const login = catchAsyncErrors(async (req, res, next) => {
    ...
    if(!user) {
        return next(new ErrorHandler("Invalid email or password. (Email)", 400));
    }

    const isPasswordMatched = await user.comparePassword(password);
    if(isPasswordMatched) {
        return next(new ErrorHandler("Invalid email or password. (password)", 400));
    }
...
export const updatePassword = catchAsyncErrors(async (req, res, next) => {
    ...
    if(req.body.newPassword !== req.body.confirmPassword) {
        return next(new ErrorHandler("New password & confirm passowrd do not match.", 400));
    }
