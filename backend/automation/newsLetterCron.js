  ...
  cron.schedule("*/1 * * * *", async () => {
    ...
    for (const job of jobs) {
      try {
        ...
        // variable to track if emails were sent for this job
        ...
        for (const user of filteredUsers) {
          ...
          // If at least one email is sent, set emailsSent to true
          emailsSent = true;
        }

        // Set newsLettersSent to true only if emails were sent to users
        if(emailsSent){
